...
export default factories.createCoreController('api::product-sale.product-sale', {
  async updateAmount(ctx) {
    const productSaleId = ctx.params.id
  ...
  async updateTotalAvailable(ctx) {
    ...
    return await strapi.db.transaction(async ({ trx, rollback, commit, onCommit, onRollback }) => {
      strapi.log.info(
        `[controllers][product-sale][updateTotalAvailability] productSaleId '${productSaleId}' newTotalAvailable: '${newTotalAvailable}'`
      )
      const productSaleEntity = await strapi.db.query('api::product-sale.product-sale').findOne({
        where: { id: productSaleId }
      })
      strapi.log.info(
        '[controllers][product-sale][updateTotalAvailability] productSaleEntity',
        productSaleEntity
      )

      const delta = productSaleEntity.total_available - newTotalAvailable
      if (
        newTotalAvailable < productSaleEntity.total_available &&
        delta < productSaleEntity.current_available
      ) {
        return ctx.badRequest('The product will get to a negative available_quantity')
      }

      const productSaleEntityUpdated = await strapi.entityService.update(
        'api::product-sale.product-sale',
        productSaleId,
        {
          data: {
